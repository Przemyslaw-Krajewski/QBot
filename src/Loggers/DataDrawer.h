...
public:
	static void drawState(State t_State, std::string name, int t_blockSize = 8);
...
private:
	inline static void drawBlock(cv::Mat *mat, int t_blockSize, cv::Point t_point, cv::Scalar t_color);
	inline static void drawBorderedBlock(cv::Mat *mat, int t_blockSize, cv::Point t_point, cv::Scalar t_color);
